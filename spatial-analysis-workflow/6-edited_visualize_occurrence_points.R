...
data_out <- "visualize_taxon_points_iucngbif"
...
if(file.exists(file.path(standard_occ,"justgbif_manual_point_edits.csv"))){
  "You've already created this file; if you'd like to overwrite it, run the 'else' statement manually"
} else {
  write.csv(edits, file.path(standard_occ,
                             "justgbif_manual_point_edits.csv"), row.names = F)
}
...
for(i in 1:length(taxa_cycle)){
  ...
  try(final_map <- leaflet() %>%
      ...
      it marks where ex situ germplasm was originally collected in the wild!",
        ...
        addCircleMarkers(data = taxon_now, ~decimalLongitude, ~decimalLatitude,
                        ...
                        # ########################################## # "<b>Number of individuals:<b>",individualCount,"<b/>",
                           "<b>UID:</b> ",UID),
  ...
  ## save map
  try(htmlwidgets::saveWidget(final_map, file.path(standard_occ,data_out,
                                                   paste0(taxa_cycle[i], 
                                                          "__justgbif_map-for-vetting.html"))))
